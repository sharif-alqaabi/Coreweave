#!/bin/bash
# Live training for the demo video: one real round from the hand-written rules_v0 on a 30-lead slice,
...
python3 - <<'PY'
import json
leads = [l for l in json.load(open("data/golden/train_llm_only.json")) if l["dataset"] == "OSD-255"]
json.dump(leads, open("data/golden/train_video.json", "w"), indent=1)
from collections import Counter; print(f"training slice: {len(leads)} leads from OSD-255,", dict(Counter(l['label'] for l in leads)))
PY
echo "=== round 0: critic judges with rules_v0, misses go to three architects, tournament, promotion ==="
python3 -u loop.py --iterations 1 --train data/golden/train_video.json --table data/raw/OSD-255_differential_expression.csv \
